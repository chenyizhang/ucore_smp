...
entryother: entryother.S
	$(CC) $(CFLAGS) -fno-pic -nostdinc -I ./kern/init/ -I ./kern/mm/ -c entryother.S
	$(V)$(LD) $(LDFLAGS) -N -e start -Ttext 0x7000 -o bootblockother.o entryother.o
...
$(kernel): tools/kernel.ld entryother

$(kernel): $(KOBJS) 
	@echo + ld $@
	$(V)$(LD) $(LDFLAGS) -T tools/kernel.ld -o $@ $(KOBJS) -b binary entryother 
...
QEMUOPTS = -hda $(UCOREIMG) -smp 4 -drive file=$(SWAPIMG),media=disk,cache=writeback -drive file=$(SFSIMG),media=disk,cache=writeback >/tmp/qemu-log.txt 
...
qemu: $(UCOREIMG) $(SWAPIMG) $(SFSIMG)
	$(V)$(QEMU) -no-reboot -d in_asm,int,cpu_reset -D a.log -serial stdio -m 512 $(QEMUOPTS) -parallel null
